...
Turning first to LENA's automated measure, a two-sample t-test shows that despite wide variability in the number of words children hear (Range: 195--992 words\textsubscript{blind}, 238--804 words\textsubscript{sighted}), blind and sighted children do not differ in language input quantity (\emph{t}(28) = -1.19, \emph{p} = .245). If we instead measure this using word counts from the transcriptions of the audio recordings, we find parallel results: blind and sighted children do not differ in language input quantity (\emph{t}(30.00) = 0.48, \emph{p} = .636); see Figure \ref{fig:quantity-plots}.
...
Next, we ask whether the language environments of blind vs.~sighted participants differ in the amount of interaction with the child, by comparing blind the proportion of child-directed speech and the number of conversational turns. Both measures were normally distributed (Prop. CDS: W = 0.97, \emph{p} = .967; CTC: W = 0.88, \emph{p} = .878). This set of analyses involves two tests, so our Bonferroni-corrected threshold for significance is \emph{p} \textless{} 0.03. Paired t-test revealed no significant difference in the proportion of child-directed speech (\emph{t} = 0.12, \emph{p} = .903) or in conversational turn counts to blind children versus to sighted children .
...
For linguistic features, we measure type-token ratio and mean length of utterance, two variables derived from the manual annotations. Because these variables met the normality assumption (TTR: W = 0.95, \emph{p} = .946; MLU: (W = 0.90, \emph{p} = .897)), we performed paired t-tests. Again, Bonferroni-corrected significance was set to \emph{p} \textless{} 0.03. Results indicated that there was no significant difference in type-token ratio between the two groups (\emph{t}(15) = 0.15, \emph{p} = .880). For MLU, we found that utterances were slightly longer to blind children than to their sighted peers (\emph{t}(15) = 0.25, \emph{p} = .805), but this difference does not survive Bonferroni correction; see Figure \ref{fig:linguistic-plots}).

\hypertarget{conceptual-features-1}{%
\subsubsection{Conceptual Features}\label{conceptual-features-1}}

Lastly, we compared three measures of the conceptual features of language input: the proportion of temporally displaced verbs, the distribution of Child-Body-Object Interaction ratings across words in the input, and the proportion of highly visual words. This set of analyses involves three tests, so our Bonferroni-corrected threshold for significance is \emph{p} \textless{} 0.02. Because the proportion of displaced verbs does not follow a normal distribution (W = 0.98, \emph{p} = .978), we tested this measure with a paired Wilcoxon test: we find that blind children hear proportionally more displaced verbs than sighted children (\emph{W} = 17, \emph{p} = .006). Next, we compared the distribution of CBOI ratings in word tokens in blind children's input to that in sighted children's input using a two-sample Kilgomorov-Smirnov test. These distributions significantly differ (D = 0.98, \emph{p} \textless{} .001). Descriptively, low CBOI words were more common in language input to blind children, and high CBOI words were more common in language input to sighted children; see Figure \ref{fig:conceptual-plots}. For the proportion of highly visual words, a Shapiro-Wilks test showed that this variable was normally distributed (W = 0.98, \emph{p} = .984). A paired t-test found no significant difference across groups in the proportion of highly visual words (\emph{t}(15) = 0.55, \emph{p} = .591).
...
\hline
Manual Word Count & Blind \textasciitilde{} Sighted & .636 & \\
\hline
Prop. Child-Directed Speech & Blind \textasciitilde{} Sighted & .903 & \\
\hline
Conversational Turn Count & Blind \textasciitilde{} Sighted & .096 & \\
\hline
Type-Token Ratio & Blind \textasciitilde{} Sighted & .880 & \\
\hline
Mean Length of Utterance & Blind > Sighted & .805 & \\
\hline
Prop. Displaced & Blind > Sighted & .006* & *\\
\hline
Child-Body-Object Interaction & Blind < Sighted & < .001* & *\\
\hline
Prop. Visual & Blind \textasciitilde{} Sighted & .591 & \\
\hline
\end{tabular}
\end{table}
...
For MLU, we had expected to find lower MLU in language input to blind children relative to sighted children. Parents of children with disabilities (Chernyak, n.d.; including parents of blind children! e.g., FamilyConnect, n.d.) are often advised to use shorter, simpler sentences with their children, and correspondingly, previous work finds that parents of children with disabilities tend to find that parents use shorter, simpler utterances (e.g., Down syndrome, \textbf{lorang2019?}; hearing loss, Dirks et al., 2020). In many cases, however, this advice is not supported by the literature; evidence suggests that longer, more complex utterances are associated with better child language outcomes in both typically-developing children (Hoff \& Naigles, 2002 Mar-Apr) and children with cognitive differences (Sandbank \& Yoder, 2016). In our sample, we found similar (and perhaps even \emph{higher}) MLUs in blind children's language environment, relative to sighted children, indicating that if anything, the language environments of blind children trend towards \emph{longer} utterances.
