...
```{r corpus-size-plots, fig.cap="The amount of text that analyses are run over. Left panel shows the corpus size in words, and right panel shows the corpus size in utterances. Each bar shows the amount of speech in one child's recording, colored by whether the words/utterances were in random or high-volubility samples."}

#ec2eb: I don't actually know that we need to include these in the paper (which is why I haven't used the same color scheme, etc.) These plots just are in response to your question: how many words are actually in these analyses? Let me know if you want something like this in the paper
