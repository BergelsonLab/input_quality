...
affiliation:
  ...
  - id            : "3"
    institution   : "Department of Psychology, Harvard University, Boston, MA"
...
<!-- praise: zhenya2erin: My general impression is that this document is very well organized and easy to follow. For 90% of the code it was clear what it did and why. There was also little repetition. -->

<!-- suggestion: zhenya2erin: I highly recommend using `renv` instead of a list of packages. -->

<!-- nitpick: zhenya2erin: Chunk names could be updated -->
...
# issue: zhenya2erin: This file isn't in the repo and the dataframe doesn't seem to be used anywhere so I commented out the following line.
#  Can it be deleted?
# lotta_data <- read_csv("../data/LENA/Transcripts/Derived/lotta_data.csv")  

# question: zhenya2erin: Are some of these files subsets of another bigger one?

# praise: zhenya2erin: The READMEs in the data folders are very informative!
# issue: zhenya2erin: It is hard to guess the contents of these tables from their names.
#  Which is OK but at least a short description of the contents would be very helpful.
#  Especially the aggregation level of each.
#  I can deduce that information from the READMEs in the data folders but it is hard to consult them while reading the code.

# suggestion: zhenya2erin: To avoid repetition and make this code easier to read, I would define the path to the Transcripts directory in the setup chunk and use it here.
#  For even less repetition, I would define a function that does `read_csv(file.path(transcripts_folder, filename))`.
...
annotated_utterances <- read_csv("../data/LENA/Transcripts/Derived/annotated_utterances.csv")
...
#demo
# note: zhenya2erin: Another - slightly different way to deal with the spaces is to use `trim_ws = TRUE` which removes spaces, tabs, etc. before parsing values.
VI_matches_demo <- read_csv("../data/Demographics/VI_matches_demo.csv", na = c("", "NA", " "))
# issue: zhenya2erin: This file isn't in the repo so I commented out the following line.
#  This dataframe is used to calculate 
...
```{r functions}
# praise: zhenya2erin: I love the use of reporting functions.

report_continuous <- function(vector, string,dig=1){
  # issue: zhenya2erin: na.rm=TRUE needs to be explained. I suggest making it a parameter that defaults to FALSE and leave a comment before every call that requires setting it to TRUE.
...
get_group_means <- function(data, column, subset, unit = "",adjust=1,digits=2) {
  # issue: zhenya2erin: This function needs documentation.
  filtered_data <- data %>%
    filter({{ subset }})
  # suggestion: zhenya2erin: Create a nested function with the signature `my_apply(min, vector)` for readability.
  # issue: zhenya2erin: na.rm=TRUE needs to be explained. I would suggest making it
  #  a parameter that defaults to FALSE and leave a comment before every call
  #  that requires it to be set to TRUE.
...
make_blind_sighted_violins <- function(data, x_var, y_var, group_var, y_label, title = NULL) {
  ggplot(data = data, aes_string(x = x_var, y = y_var, color = group_var, fill = group_var)) +
    ...
    coord_cartesian(ylim = c(0, max(data[[y_var]] * 1.1)))
}
...
The early language skills of blind children highly variable [@campbell2024]. Some demonstrate age-appropriate vocabulary from the earliest stages of language learning [@landau1985; @bigelow1987; @campbell2024], while others experience substantial language delays [@campbell2024]. By adulthood, however, blind individuals become fluent language-users, demonstrating faster lexical processing skills than sighted adults [@roder2000; @roder2003; @loiotile2020]. The causes of early variability and the later ability to "catch up" remain poorly understood: what could make the language learning problem different or initially more difficult for the blind child? Here, we compare the language environments of blind children to that of their sighted peers. In doing so, we begin to untangle the role that perceptual input plays in shaping children's language environment, and better understand the interlocking factors that may contribute to variability in blind children's early language abilities.
...
[^1]: In the present study, we refrain from labeling linguistic characteristics as “quality” measures to avoid perpetuating biased standards favoring certain linguistic norms. The term “quality” has historically stigmatized input from already-marginalized (e.g., people of color, disabled groups) by framing their language as deficient (MacLeod & Demers, 2023).
...
Speakers regularly tailor their speech to communicate efficiently with the listener [@grice1975]. Across many contexts, research finds that parents are sensitive to their child's developmental level and tune language input accordingly [@snow1972; @vygotsky1978]. One example is child-directed speech, wherein parents speak to young children with exaggerated prosody and slower speech [@fernald1989; @bernsteinratner1984; @moser2022], which are in some cases helpful to the young language learner [@thiessen2005]. For instance, parents tend to repeat words more often when interacting with infants than with older children or adults [@snow1972; @fernald1993]. Communicative tailoring is also common in language input to children with disabilities, who have been found to receive simplified, more directive language input, and less interactive input compared to typically-developing children [@dirks2020; @yoshinaga-itano2020].

In addition to tailoring communication to children's developmental level, speakers also adjust their conversation in accordance to their partner's sensory access [@gergle2004; @grigoroglou2016]. In a noisy environment, speakers often adapt the acoustic-phonetic features of their speech to make it easier for their interlocutor to understand them [@hazan2011], demonstrating sensitivity to even temporary sensory conditions. When describing scenes, speakers aim to provide the information their listeners lack but avoid redundant visual description [@ostarek2019; @grice1975]. During in-lab tasks with sighted participants, participants in several studies verbally provided visually-absent cues when an object is occluded to their partner [@jara-ettinger2021; @hawkins2021; @rubio-fernandez2019]. These results suggest that adults and even infants [@senju2013; @ganea2018; @chiesa2015] can flexibly adapt communication to the visual and auditory abilities of their partner.
...
# issue: zhenya2erin: I set this variable to NA because I commented out code that creates VIHI_transcripts_messy. This should definitely not stay like that because this number is used in the text.
perc_random_silent <- NA
  # ((
...
# nitpick: zhenya2erin: I understand that `_all` refers to not being split into blind and sighted, but the names make me think that VI_matches_demo_all has more data that VI_matches_demo.
# nitpick: zhenya2erin: It might make this document cleaner if this was done in the preprocessing step.
...
# issue: zhenya2erin: The code below is very hard to read which makes it easy to miss an error somewhere.
#  A function that takes a label and a column name and returns a row of the table would make it very clear.
...
# issue: zhenya2erin: These two block have almost identical code. It is hard to say for sure though. A function would make the code more clear.
...
cowplot::plot_grid(word_counts_plot,utterance_counts_plot, rel_widths = c(1,1.5), labels = c('A', 'B'))
```
...
# issue: zhenya2erin:
#  1. This code is hard to read and needs to be refactored. E.g., like this (not tested):
#
#     ```
#     displaced_agreement <-
#       displacement_subset_for_agreement %>%
#       filter(manual_temporality == "displaced") %>%
#       {sum(.$temporality == .$manual_temporality) / nrow(.)}
#     ```
#  2. There needs to be a comment about what is in `*_subset_for_agreement` tables and what temporality is. The latter is defined in the text but it isn't clear that I need to look for it.
#
#  3. There needs to be a comment explaining what NAs mean and why it is OK to drop them.
...
We also analyzed the syntactic complexity of children's language input, approximated as mean utterance length in morphemes. Both type-token ratio and mean length of utterance in speech to infants have been found to be consistent within individual caretakers, in and out of lab settings [@stevenson1986]. Each utterance was tokenized into morphemes using the 'morphemepiece' R package [@bratt2022]. We then calculated the mean length of utterance (number of morphemes) per speaker in each audio recording. We manually checked utterance length in a random subset of 10% of the utterances (n = `r nrow(MLU_subset_for_agreement)`), which yielded a intra-class correlation coefficient of `r MLU_agreement$value` agreement with the udpipe approach (*p* `r printp(MLU_agreement$p.value)`), indicating high consistency.
...
MWC_shapiro<-shapiro.test(manual_word_tokens$tokens)
...
<!-- issue: zhenya2erin: Long inline code is very hard to read. I would suggest calculating all these numbers in an R chunk first. Could the `report_*` funcitons be used here? -->
...
CTC_shapiro <- shapiro.test(LENA_counts$CTC_stand)
...
# issue: zhenya2erin: Having multiple vectors that need to match each other is a recipe for disaster, in my opinion. This definitely needs to be refactored. One way to do this:
#  - Instead of using adjust, convert LENA_counts to per hour during preprocessing/loading.
#  - All the hard-coded values could be put into a single dataframe or a vector of named lists.
#  - Test results could be put into a named list.
#  - Calculate every calculated vector using that dataframe with map/apply/mutate, etc.
#  - ps_df should be defined in a way that puts column names next to the corresponding vectors.
...
We wish to highlight again how much variability there is *within* groups and how much consistency there is *between* groups. One could imagine a world in which the language environments of blind and sighted children are radically different from each other. Our data do not support that hypothesis. Rather, we find similarity in quantity and interaction, alongside modest differences in linguistic and conceptual properties. That is, in line with recent work highlighting immense *within*-group variability across many different socio-cultural and linguistic contexts [@bergelson2023], our blind and sighted groups here have large within-group variability but very few between-group differences. Despite strikingly different visual experiences, young blind and sighted learners have at best modest differences in the speech environments that surrounded them.
