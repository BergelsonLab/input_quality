...
# ec2eb: updated address//eb2ec: boston->cambridge//added 'young' to title (plz propogage to letter)
---
...
**Purpose:** We compared everyday language input to young congenitally-blind children with no additional disabilities (N=15, 6--30mo., M:16mo.) and demographically-matched sighted peers (N=15, 6--31mo., M:16mo.) By studying whether the language input of blind children differs from their sighted peers, we aimed to determine whether, in principle, the language acquisition patterns observed in blind and sighted children could be explained by aspects of the speech they hear.

**Methods:** LENA recorders captured the auditory language environment in children's homes. Speech in these recordings was then analyzed with a mix of automated and manually-transcribed measures across various subsets and dimensions of language input. These included measures of quantity (adult words), interaction (conversational turns and child-directed speech), linguistic properties (lexical diversity and mean length of utterance), and conceptual features (talk centered around the here-and-now; talk focused on visual referents that would be inaccessible to the blind but not sighted children).

**Results:** Overall, we found broad similarity across groups in speech quantity, interaction, and linguistic properties. The only exception was that blind children's language environments contained slightly but significantly more talk about past/future/hypothetical events than sighted children's input; both groups received equivalent quantities of "visual" speech input.

**Conclusions:** The findings challenge the notion that blind children's language input diverges substantially from sighted children's in a way that would place them at a disadvantage. They suggest instead that blind children receive rich and complex language input that readily supports their language development, with open questions remaining regarding how this input may be differentially leveraged by language learners in early childhood.
<!--ec2eb: added details to abstract. word limit of 150–250 words for JSLHR abstract. Currently at 196, so we have some room to add//eb2ec: now it's at about 250, see what you think; i'm trying to make this as informative as possible to ppl who will only be able to access the abstract (e.g. many ASHA folks)-->

# Introduction

The early language skills of blind children highly variable. Some children demonstrate age-appropriate vocabulary and grammar from the earliest stages of language learning, while others experience substantial language delays [@landau1985; @bigelow1987; @campbell2024]. By adulthood, however, blind individuals are fluent language-users, even demonstrating, e.g. faster lexical processing skills than sighted adults [@roder2000; @roder2003; @loiotile2020, though c.f., @sak-wernicka2017 for discussion of possible pragmatic differences]. The causes of early variability and the potential ability (or need) to "catch up" remain poorly understood: what could make the language learning problem different or initially more difficult for the blind child? Here, we compare the language environments of blind children to that of their sighted peers. In doing so, we begin to untangle the role that perceptual input plays in shaping children's language environment, and better understand the interlocking factors that may contribute to variability in blind children's early language abilities.
...
<!-- * ec2eb:added some more specific info here. Don't want to get too in depth about blind adults' language, but I think this current para feels more balanced?//eb2ec: yes better, i did a little revamping just so you're not citing our paper 3 times int he first 3 sentences -->
...
From this (necessarily abridged) review, it appears that many factors in the language input alone link to how sighted children learn about the world and language, but that children also learn from sensory, conceptual, and social knowledge. Many cues for word learning are visual: for example, empirical work finds that sighted children can leverage visual information like parental gaze, shared visual attention [@tomasello1986], pointing [@lucca2018], and the presence of salient objects in the visual field [@yu2012]. Because these visual cues are inaccessible to blind children, language input may take on a larger role in the discovery of word meaning [@campbell2022]. Syntactic structure in particular provides critical cues to word meaning, such as the relationship between two entities that aren't within reach, or are intrinsically unobservable or ambiguous [@gleitman1990]. But in order to evaluate whether language input plays a larger role for blind versus sighted children's learning, it would be worth first establishing whether blind and sighted children's language input differs. That is, children with different sensory access could differentially make use of the same kind of language input, or they could apply the same learning mechanisms to input with different properties--a debate carried over from work with typically-sighted children [@newport1977]. Either way, characterizing the input across potentially relevant dimensions is a helpful first step. 
...
ec2eb: added gleitman1995 citation and  a few citations to newport1977 throughout intro.//eb2ec: looks good  -->
...
<!---ec2eb:footnote 2 is new//eb2ec: looks good-->
...
<!-- ec2eb: I decided to cut the overall agreement values entirely (but I pasted them below) because I'm not sure it makes sense. Talked with Lilli, and the overall % agreement and kappa refer to reliability for all closed vocabulary tiers. That includes addressee (which we describe below) and then all the child production tiers (excluding Genia's, so: vocal maturity, whether it contained a word, whether it contained multiple words). Since we only use addressee, and that's described below, I'm not sure the "overall" values are useful. We don't have a reliability measure for transcription: "no transcriptions were compared, those are just cleaned up by the Superchecker" -->
<!-- And then this is the text I cut: ", as described below. Agreement across all closed-vocabulary tiers (addressee, ) was 95%, with a kappa of 0.945 [CI: 0.939--0.95]"//eb2ec: looks good with new changes -->

## Extracting Measures of Language Input 

To go from our dimensions of interest (quantity, interactiveness, linguistic, conceptual), to quantifiable properties, we used a combination of automated measures [generated by the proprietary LENA algorithm, @xu2009] and manual measures (generated from the transcriptions and classifications made by our trained annotators). Altogether, this corpus presently includes approximately `r round(sum(LENA_counts$total_time_dur) / 60 / 60)` hours of audio, `r nrow(VITD_transcripts)` utterances, and `r nrow(VITD_LENA_words)` words. LENA measures were calculated over the whole day, and then normalized by recording length. Transcription-based quantity and interactiveness analyses were conducted on the random samples only, to capture a more representative estimate. <!--ec2eb:previous 2 sentences are edited//eb2ec: looks good-->Linguistic and conceptual analyses were conducted on all available annotations in order to maximize the amount of speech over which we could calculate them. These measures are described below and summarized in Table \@ref(tab:analysis-summary-table).
...
<!-- ec2eb: The reason that I'm doing this instead is because, like Jessica Montag said, TTR accumulates non-linearly, so different numbers of words result  (even if it's for totally unrelated reasons: like we happened to randomly sample chattier minutes for some kids than others)

Reading back through the Jessica Montag thread: it looks like she said either options B (my approach with 100-word samples) or C (her approach with 1000-word samples) would be sensible. She did not recommend option A (just raw TTR over the entire (variable size) number of words). 

I still prefer my 100-word method to better maintain contiguity. You're right that we sample in the same way across children in our groups, but our sampling methods--and particularly the random samples-- necessarily mean that some kids' samples are closer together than other kids' samples. 

The CogSci paper makes the good point that "Children’s language input may or may not be generally consistent over time, i.e., how much consistency is there in the amount of language encountered in different hours throughout the day. Further that consistency itself may vary across families such that for some children the amount of language they hear may be fairly consistent across the day such that different hours contain approximately equal amounts of speech, which for other children some hour might contain large amounts of speech while others contain very little. " and for that reason, she strongly advocates for taking contiguous samples. 

Additionally, we have less transcribed per child than she does. On the lower end, some kids hear <1000 words in our 40 transcribed minutes, so we don't have the ability to measure per 1000 words.

100-word samples is also close to what's often done for TTR in clinical practice, which is to extract TTR from a 100-word sample. By mirroring that, our values are more immediately comparable. You're right that we could just report it as per 100 words, but since TTR changes as a function of sample size, our estimates would look artificially deflated, relative to a typical clinical speech sample TTR. For us, the raw TTR is less than half the 100-word TTR (0.27 vs. 0.63)

With alllllll that said, if we do just raw TTR (which I think is a noisier measure), we get the same answer: no difference. Added that in a footnote, but please feel free to move or re-word. I don't know if Methods or Results is the right place for that//eb2ec: okay, this seems fine and well-reasoned. i don't see the footnote though. i'd put it in methods, right after this preceding sentence, something like "Computing TTR over the entire sample instead of averaging over 100-word bins rendered the same pattern of results"-->
...
We also analyzed the syntactic complexity of children's language input, approximated as mean utterance length in morphemes. Each utterance in a child's input was tokenized into morphemes using the 'morphemepiece' R package [@bratt2022]. We then calculated the mean length of utterance (number of morphemes) in each audio recording. We manually checked utterance length in a random subset of 10% of the utterances (n = `r nrow(MLU_subset_for_agreement)` utterances), which yielded a intra-class correlation coefficient of `r MLU_agreement$value` agreement with the morphemepiece approach (*p* `r printp(MLU_agreement$p.value)`), indicating high consistency.
...
# eb2ec: where have all dots&whiskers gonnnneee, long time passsinggg i wanna knowww where have all dots&whiskers goneeee long timeee agooo telllmeetellmee [all figures]
...
<!-- ec2eb: these are with the raw TTR. Can uncomment if you want to run it, but statistical values are nearly identical (and still not significant). -->
<!-- Raw TTR: (*t*(`r raw_TTR_test$parameter`) = `r raw_TTR_test$statistic`, *p* `r papaja::printp(raw_TTR_test$p.value, add_equals=TRUE)`). Children in our samples hear on average `r round(mean(raw_TTR_stats$mean),2) * 100` unique words per hundred words -->
...
<!-- ec2eb: added huber citation//eb2ec: great -->
