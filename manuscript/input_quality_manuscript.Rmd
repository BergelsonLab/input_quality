...
author:
  - name          : "Erin Campbell"
    affiliation   : "1"
    corresponding : yes    
    # chore: zhenya2erin: Reminder to update :-)
...
<!-- nitpick: zhenya2erin: Chunk names could be updated. For example, `libraries` does a lot more than installing/loading packages. -->
...
# suggestion: zhenya2erin: Global settings like this are easier to not miss if they're at the top of the file and maybe in a separate chunk.
#  Another good place is the params section in the YAML header.
preprocess <- FALSE
new_graphs <- FALSE
...
# nitpick: zhenya2erin: Name `participant_summary` made me think that the table would contain summary statistics for each participant. I would rename it to participants_summary, for example.
...
# suggestion: zhenya2erin: I would move the reporting functions to a separate chunk.
...
<!-- nitpick: eb2ec: AWC_check and compare_AWC sound like the same thing. I would use something like AWC_shapiro and AWC_paired_ttest. This applies to all the tests/summaries in the document. -->

```{r compare-AWC}
# issue: eb2ec: There needs to be a comment describing AWC_stand and
#  the contents of manual_word_tokens
# praise: eb2ec: The code is short and clear so the repetition is not an issue.
...
<!-- issue: eb2ec: Long inline code is very hard to read. I would suggest calculating all these numbers in an R chunk first. Could the `report_*` funcitons be used here? -->
...
<!-- issue: eb2ec: This needs to be a function. It is very hard to tell whether there are any difference in the plots. This goes for every chunk with plots in the document. -->
...
get_group_means <- function(data, column, subset, unit = "",adjust=1,digits=2) {
  ...
  # suggestion: zhenya2erin: Do the rounding outside of glue::glue() for readability
  #  and consistency.
