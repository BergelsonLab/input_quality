...
utterances_only<-VITD_transcripts %>% 
  ...
  mutate(utterance = str_replace_all(utterance, "<\\w+>", ""),
         utterance = str_replace_all(utterance, "=!\\w+\\s*", ""),
         utterance = str_replace_all(utterance, "[[:punct:]&&[^']]", ""),# remove punctuation from utterances
         utt_num = 1:nrow(.)) %>%
  filter(!grepl("utt@|inq@", speaker))
...
manually_coded_MLU_subset <- read.csv("data/LENA/Transcripts/Derived/manually_coded_MLU_subset.csv")


# Calculate agreement between the manual coding and original data
MLU_subset_for_agreement <- left_join((manually_coded_MLU_subset %>% distinct(utterance,utt_num,.keep_all = TRUE)),
                                           random_MLU_subset, 
                                           by = c("utterance","utt_num")) %>%
  dplyr::select(utterance,morphemecount, manual_morpheme_count) %>%
  filter(!is.na(manual_morpheme_count)) %>%
  mutate(morpheme_diff = morphemecount-manual_morpheme_count,
...
sensory_props_wide <- VITD_LENA_words %>% 
  ...
  mutate(Modality = case_when(Max_strength.perceptual<3.5 ~ "Amodal",
...
# random_modality_subset <- VITD_LENA_words%>%
#   filter(!is.na(Visual.mean)) %>%
#   sample_n(round((0.1 * nrow(VITD_LENA_words%>%
#                                filter(!is.na(Visual.mean)))))) %>%
#   mutate(Modality = case_when(Exclusivity.perceptual <= .5 ~ "Multimodal",
#                               Max_strength.perceptual<3.5 ~ "Amodal",
#                               Max_strength.perceptual>4  &  Exclusivity.perceptual <= .3 ~ Dominant.perceptual,
#                               TRUE~Dominant.perceptual)) 
random_modality_subset <- read_csv("data/LENA/Transcripts/Derived/random_modality_subset.csv")

# secret_random_modality_subset <- random_modality_subset %>%
#   dplyr::select(Word) %>%
#   mutate(manual_modality = "NA")
# # Export the random subset to a CSV file for manual coding in Excel
# write_csv(secret_random_modality_subset, "data/LENA/Transcripts/Derived/secret_random_modality_subset.csv")
# Once manual coding is done in Excel, read the manually coded CSV file back into R
manually_coded_modality_subset <- read_csv("data/LENA/Transcripts/Derived/manually_coded_modality_sample.csv")

# Calculate agreement between the manual coding and original data
modality_subset_for_agreement <- left_join((random_modality_subset %>% distinct(Word,.keep_all = TRUE)),
                                           manually_coded_modality_subset, 
                                                 by = c("Word")) %>%
  dplyr::select(Word, Modality, manual_modality) 

# Calculate Percent Agreement
modality_agreement <- sum(modality_subset_for_agreement$Modality == modality_subset_for_agreement$manual_modality) / nrow(modality_subset_for_agreement)
visual_agreement <- sum((modality_subset_for_agreement%>% filter(manual_modality == "Visual"))$Modality == (modality_subset_for_agreement%>% filter(manual_modality == "Visual"))$manual_modality) / nrow(modality_subset_for_agreement %>% filter(manual_modality == "Visual"))
...
# Once manual coding is done in Excel, read the manually coded CSV file back into R
random_displacement_subset <- read_csv("data/LENA/Transcripts/Derived/random_displacement_subset.csv")

manually_coded_displacement_subset <- read_csv("data/LENA/Transcripts/Derived/manually_coded_displacement_subset.csv")
# Calculate agreement between the manual coding and original data
displacement_subset_for_agreement <- left_join(random_displacement_subset,
(manually_coded_displacement_subset %>% mutate(token_id=as.character(token_id),
                                               head_token_id=as.character(head_token_id))), 
by = c("paragraph_id", "sentence_id", "sentence","token_id", "token", "lemma","head_token_id","dep_rel","deps","misc")) %>%
  dplyr::select(doc_id, paragraph_id, sentence_id, sentence, temporality, manual_temporality) 

# Calculate Percent Agreement
displacement_agreement <- sum(displacement_subset_for_agreement$temporality == displacement_subset_for_agreement$manual_temporality, na.rm=TRUE) / nrow(displacement_subset_for_agreement %>% filter(!is.na(manual_temporality)))
displaced_agreement <- sum((displacement_subset_for_agreement %>% filter(manual_temporality == "displaced"))$temporality == (displacement_subset_for_agreement%>% filter(manual_temporality == "displaced"))$manual_temporality) / nrow(displacement_subset_for_agreement %>% filter(manual_temporality == "displaced"))
