...
hereandnow_plot <- ggplot(data = temporality_props_wide, aes(x = group,
  ...
  stat_summary(
    ...
    alpha = 0.8) +
  geom_linerange(aes(
    ymin = (mean(prop_displaced) - std.error(prop_displaced)),
    ymax = (mean(prop_displaced) + std.error(prop_displaced))), color="black")+
  scale_x_discrete(
    labels = c(
      "TD" = "Sighted",
      "VI" = "Blind")) +
...
```{r variable-relationships, fig.cap="Kendall's Tau correlations between subject-level language input variables. Significant correlations are marked with asterisks.", fig.height=6,fig.width=6}
...
# dplyr::rename(CDI_age_in_days=age)
#
predict_AVA <- lm(AVA_standard_score ~ (LENA_age_in_days*group +AWC*group + TTR*group + MLU*group  + CBOI_Mean*group), data = lotta_data)


predict_AVA_full <- lm(AVA_standard_score ~ (LENA_age_in_days +AWC + TTR + MLU  + CBOI_Mean + prop_displaced + prop_CDS + group), data = lotta_data)

predict_AVA <- lm(AVA_standard_score ~ (LENA_age_in_days*group +AWC*LENA_age_in_days + TTR*LENA_age_in_days + MLU*LENA_age_in_days  + CBOI_Mean*LENA_age_in_days), data = lotta_data)

AVA_AIC_selected <- lm(formula = AVA_standard_score ~ AWC + MLU, data = lotta_data)


predict_CVC_group_interaction <- lm(CVC ~ (LENA_age_in_days*group +AWC*group + TTR*group + MLU*group  + CBOI_Mean*group), data = lotta_data)
CVC_AIC_selected<- lm(formula = CVC ~ LENA_age_in_days + group + AWC + TTR + CBOI_Mean + LENA_age_in_days:group + LENA_age_in_days:AWC + LENA_age_in_days:TTR + LENA_age_in_days:CBOI_Mean, data = lotta_data)

predict_CVC_full <- lm(CVC ~ (LENA_age_in_days +AWC + TTR + MLU  + CBOI_Mean + prop_displaced + prop_CDS + group), data = lotta_data)
CVC_AIC_selected <- lm(formula = CVC ~ LENA_age_in_days + AWC + MLU + prop_displaced +
    group, data = lotta_data)
predict_CVC_age_interaction <- lm(CVC ~ (LENA_age_in_days*group +AWC*LENA_age_in_days + TTR*LENA_age_in_days + MLU*LENA_age_in_days  + CBOI_Mean*LENA_age_in_days), data = lotta_data)
CVC_AIC_selected<- lm(formula = CVC ~ LENA_age_in_days + group + AWC + TTR + CBOI_Mean + LENA_age_in_days:group + LENA_age_in_days:AWC + LENA_age_in_days:TTR + LENA_age_in_days:CBOI_Mean, data = lotta_data)
summary(CVC_AIC_selected)
...
Likewise, we quantified interactiveness in two ways: through the LENA-estimated conversational turn count, and through the proportion of child-directed speech in our manual annotations. Again, we found no differences across groups in the amount of parent-child interaction. This finding runs counter to previous research; other studies report *less* interaction in dyads where the child is blind [@rowland1984 (N=5, "With the exception of Maria, all the subjects were severely developmentally delayed.", films were made in home); @perez-pereira2001 (N=2, "60-minute video recording sessions at home with the second author or a research assistant as participant observers"), @moore1994 (n=8, "visited at home at around 18 months of age and videorecordings were made of each child interacting with a familiar caretaker"), @kekelis1984 (n=4, parents initiate topics more often, more imperatives, non-here-and-now topics being relatively rare, "visually impaired but otherwise intact", "Approximately 30 minutes of the home visit were videotaped and one hour was audiotaped. Parents had been instructed that we were interested in observing a" chunk of everyday life" in their homes, and over the course of the study we observed a variety of activities. The tapes were transcribed and checked, and contextual notes taken during the home visits were incorporated."); ] @tadic2013 @preisler1991 @loots2003 @grumi2021 @campbell2003 @baird1997 @anderson1993 
taps into a slightly different latent construct than our measures
<!-- ec2gl could this be because other studies collected data in lab?? -->
...
@moore1994 (n=8, "visited at home at around 18 months of age and videorecordings were made of each child interacting with a familiar caretaker" "A few minutes warm-up was given before the video-camera was switched on. The parents did not appear to be inhibited by the presence of the camera and most reported subsequently that they forgot about it very
quickly. ") -- MORE DISPLACED LANGUAGE / also less child-initiation
