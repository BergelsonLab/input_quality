...
LENA_counts <- read.csv("data/Automated/LENA_counts.csv")
sensory_props <- read.csv("data/Transcripts/sensory_props.csv")
MLUs <- read.csv("data/Transcripts/MLUs.csv")
manual_word_TTR <- read.csv("data/Transcripts/manual_word_TTR.csv")
xds_props <- read.csv("data/Transcripts/xds_props.csv")
...
temporality_props %>%
  ...
  summarise(prop = mean(prop, na.rm = TRUE)) %>%
  ggplot(aes(fill = factor(verb_temporality, levels=c("uncategorized","past","present")),
                                 y = prop,
                                 x = group)) +
  geom_bar(position = "stack", stat = "identity")  +
  ylab ("Proportion of utterances\nby verb temporality") +
  ...
  scale_fill_manual(name="Verb Tense",
                    labels=c("uncategorized", "subjunctive,\nfuture, or past","present"),
                    values=c("#A5ACA5","#FFF170","#BCE784"))+
  scale_x_discrete(labels =                                                   c("VI" = "Blind", "TD" = "Sighted")) +
  xlab(NULL) 
```
The last conceptual feature we examined is the displacement of events discussed in children's linguistic environment. That is, we tried to ascertain whether blind and sighted children receive a different amount of "here-and-now" commentary related to the events happening around them. To do this, we used the udpipe package [CITE] to tag the transcriptions with their parts of speech and other features of the utterance, such as verb form, tense, and mood. Using the features tagged to the first verb or auxiliary in each utterance (a proxy for the hierarchically dominant verb), we assigned each utterance a temporality value: utterances tagged *non-present* describe events that take place in the past, future, or irrealis space, while utterances tagged *present* describe current, ongoing events. A small amount of utterances (n = `r verbs_only, count(temporality = "uncategorized)`) were left uncat 
```{r cboi}
CBOI_plot <-
  ggplot(data = subj_means, aes(
    x = group,
    y = mean,
    color = group,
    fill = group)) +
  ...
  geom_violin(alpha = .5, trim = FALSE) +
  theme_classic() +
  labs(y = "Mean Child-Body-Object\nInteraction Rating (1-7)", x = NULL, element_text(size = 14)) +
  theme(
    legend.title = element_blank(),
    legend.position = "none") +
  scale_color_manual(
  ...
  stat_summary(
    ...
    alpha = 0.8) +
  geom_linerange(aes(
    ymin = (mean(mean) - std.error(mean)),
    ymax = (mean(mean) - std.error(mean))), color="black")+
  scale_x_discrete(
    labels = c(
      "TD" = "Sighted",
      "VI" = "Blind")) +
  coord_cartesian(ylim = c(0, (max(subj_means$mean * 1.1)))) +
  theme(text = element_text(size = 14),
    axis.text.x = element_text(angle = 45, hjust = 1)) 
CBOI_plot
```
...
lotta_data <- LENA_counts %>%
  ...
  left_join(xds_props_wide %>% select(-total)) %>%
  left_join(sensory_props %>% select(-total))%>% 
...
predict_CDI <- lm(diff_age_from_expected ~ CDI_age_in_days + AWC + tokens + TTR + CTC + prop_CDS + MLU + group + group:CDI_age_in_days + group:AWC + group:tokens + group:TTR + group:CTC + group:prop_CDS + group:MLU, data = lotta_data)

summary(predict_CDI)
MASS::stepAIC(predict_CDI)
AIC_selected <- lm(formula = diff_age_from_expected ~ group + AWC + tokens + 
    TTR + CTC + prop_CDS + MLU + group:tokens, 
    data = lotta_data)
summary(AIC_selected)

predict_CDI_TD <- lm(diff_age_from_expected ~  tokens + CTC+  prop_CDS + MLU, data = (lotta_data %>% filter(group=="TD")))
...
predict_CDI_VI <- lm(diff_age_from_expected ~ CDI_age_in_days + tokens + TTR + CTC + prop_CDS + MLU, data = (lotta_data %>% filter(group=="VI")))
MASS::stepAIC(predict_CDI_VI)
AIC_selected_VI <- lm(formula = diff_age_from_expected ~ CDI_age_in_days*MLU, 
    data = (lotta_data %>% filter(group == "VI")))
summary(AIC_selected_VI)


trying_this_one <- lm(formula = diff_age_from_expected ~ CDI_age_in_days+ group*MLU, 
    data = lotta_data)
...
VI_cors <- cor(lotta_data %>% filter(group=="VI") %>% select(MLU,AWC,tokens,CTC,prop_CDS,TTR))
TD_cors <- cor(lotta_data %>% filter(group=="TD") %>% select(MLU,AWC,tokens,CTC,prop_CDS,TTR))
