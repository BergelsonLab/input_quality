...
```{r variable-relationships, fig.cap="Kendall's Tau correlations between subject-level language input variables. Significant correlations are marked with asterisks.", fig.height=5,fig.width=6,include=FALSE}
...
associations_VI <- corrplot(cors_VI, type = "lower",method="color", tl.col = "black",tl.cex=.8, addCoef.col="white",p.mat=cor_ps_VI, number.cex=.7,insig="n")
...
associations_TD <- corrplot(cors_TD, type = "lower",method="color", tl.col = "black",tl.cex=.8, addCoef.col="white",p.mat=cor_ps_VI, number.cex=.7,insig="n")

# write function for testing difference between correlations

compare_corrs <- function(var1, var2) {
subset_VI <- lotta_data %>% filter(group=="VI")
subset_TD <- lotta_data %>% filter(group=="TD")  
  correlation1 <- cor.test(subset_VI[[var1]],subset_VI[[var2]], method="kendall")
correlation2 <- cor.test(subset_TD[[var1]],subset_TD[[var2]], method="kendall")
diff_test <-  cocor.indep.groups(r1=correlation1$estimate, r2=correlation2$estimate, n1=15, n2=15, alternative="two.sided", alpha=0.05, conf.level=0.95, null.value=0)

glue("*r*~blind~ = {printnum(correlation1$estimate)}, *r*~sighted~ = {printnum(correlation2$estimate)}, z = {papaja::printnum(diff_test@fisher1925[['statistic']])}, *p* {papaja::printp(diff_test@fisher1925[['p.value']],add_equals = TRUE)}")
...
```{r compare-corrs, include=FALSE}
matrix_comparison <- cortest.jennrich(cors_VI,cors_TD,n1=15, n2=15)
par(mfrow=c(2,1))
associations_VI
associations_TD
```
Lastly, we also ran an exploratory analysis testing for patterns among these measures of language input. First, we re-aggregated the language input variables such that each child had a single value for each predictor; this required calculating MLU over child rather than over speaker and giving each child's input a mean child-body-object interaction rating. Next, we generated correlation matrices separately for the blind sample and the sighted sample, using Kendall's Tau correlations; see Figure \@ref(variable-relationships). We then compared correlations among variables across groups using Fisher's z test. To reiterate, this analysis is purely exploratory and descriptive in nature.

Looking across matrices, we found many similarities in how properties of children's language input patterned across groups. To highlight a few, in both samples, children who heard more adult words were involved in more conversational turns (`r compare_corrs("AWC","CTC")`), heard more child-directed speech were involved in more conversational turns (`r compare_corrs("prop_CDS","CTC")`), heard more adult words had a lower type-token ratio (`r compare_corrs("AWC","TTR")`), heard more highly visual words had higher BOI ratings (`r compare_corrs("prop_Visual","CBOI_Mean")`).
However, we also found some differences, where associations ran in the *opposite* direction:
- for blind kids but not sighted kids, higher conversational turns was associated with lower type-token ratio (`r compare_corrs("AWC","CTC")`)
- for blind kids but not sighted kids, higher BOI ratings was associated with a greater proportion of temporally displaced verbs (`r compare_corrs("prop_displaced","CBOI_Mean")`)
- for blind kids but not sighted kids, proportion of child-directed speech was associated with lower proportion of highly visual words (`r compare_corrs("prop_CDS","prop_Visual")`)


We then test whether these correlation matrices are significantly different from each other using the Jennrich test for matrix equality. We find that these (this magnitude of difference would be significant at a sample size of 29 per group).
