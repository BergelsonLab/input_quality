...
VI_group_props <- wide_transcripts %>%
  filter(xds %in% c("C","A","B","P", "O", "U"))%>%
  group_by(xds) %>%
...
```{r CDS-prop-compare}
group_proportion_table_long <- bind_rows(VI_group_proportion_table, TD_group_proportion_table)

proportion_table_subject_level <- bind_rows(proportion_table, TD_proportion_table) %>%
  dplyr::select(-incidence_percent, -counts) %>%
  pivot_wider(names_from = xds,
              values_from = xds_prop) %>% 
    mutate_at(3:8, ~replace_na(.,0)) %>%
  rename_at(3:8, ~ paste(., "DS_prop", sep = "")) %>%
  mutate(group=as.factor(str_sub(VIHI_ID, 1, 2)),
         ADS_counts = ADS_prop*total_utts,
         CDS_counts = CDS_prop*total_utts,
         ODS_counts = ODS_prop*total_utts,
         BDS_counts = BDS_prop*total_utts,
         UDS_counts = UDS_prop*total_utts,
         PDS_counts = PDS_prop*total_utts)
...
shapiro.test(proportion_table_subject_level$CDS_prop)
CDS_compare <- t.test(proportion_table_subject_level$CDS_prop ~ proportion_table_subject_level$group)
...
sensory_words_by_modality_xds_long <- sensory_words%>% 
  group_by(group, VIHI_ID, Dominant.perceptual, xds) %>%
  summarise(by_modality = n()) %>%
  left_join(sensory_words_totals) %>%
  mutate(by_modality = replace_na(by_modality,0),
         prop = by_modality/total)
  
sensory_words_by_modality_xds_wide <- sensory_words_by_modality_long%>% 
  pivot_wider(names_from = Dominant.perceptual, values_from=prop)%>%
  left_join(sensory_words_by_modality_long) 
...
TTR_test <- wilcox.test(manual_word_TTR$TTR~manual_word_TTR$group)
```
...
final_forMLU_TD <- merge(forMLU_TD_utterances, for_MLU_TD_morphemes, by="VIHI_ID", all = TRUE)
