...
VI_matches_demo[VI_matches_demo == ""]<-"Unreported"
...
VI_matches_demo$Race[VI_matches_demo$Race == "Black or African American,White"]<- "Mixed"
...
demographics_table <- make.table(dat = VI_matches_demo,
                                 strat = "match_group",
                                 ...
                                 cont.rmstat = list(c("mediqr","q1q3", "count", "miss"), c("mediqr","q1q3", "count", "miss")))
```
...
predict_CDI_full <- lm(diff_age_from_expected ~ AWC + CTC + prop_CDS + TTR + MLU  + prop_Visual + CBOI_Mean, data = (lotta_data %>% filter(group=="VI")))
vif(predict_CDI_full) # remove prop_Displaced, age
# stepAIC(predict_CDI_full) # lm(formula = AVA_standard_score ~ AWC + MLU, data = lotta_data)
...
Along the linguistic dimension, we measured type-token ratio and mean length of utterance. Type-token ratio was similar across groups, and in line with type-token ratio in other child-centered corpora [e.g., @newman2015]. However, we found slightly but significantly higher MLU in blind children's language environment.
Parents of children with disabilities (including parents of blind children! https://familyconnect.org/browse-by-age/infants-and-toddlers/growth-and-development-iandt/language-development/ https://www.wikihow.life/Teach-Your-Blind-or-Visually-Impaired-Child-to-Talk) are often advised to use shorter, simpler sentences with their children, in order to promote children's understanding. Yet, longer, more complex utterances are associated with better child language outcomes in both typically-developing children [@hoff2002] and children with cognitive differences [@sandbank2015].
