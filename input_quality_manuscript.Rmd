...
LENA_counts <- read_csv("/Volumes/pn-opus/VIHI/Scripts/LENA_Scripts/data/VIHI_its_details.csv") %>%
  filter(group == "VI" | VIHI_ID %in% TD_matches) %>%
  mutate(Age_days = as.numeric(str_sub(VIHI_ID,8,-1L))) %>%
  mutate(Age_months = Age_days*0.0328767) %>%
...
topic diversity - <https://github.com/mind-Lab/octis> adjective typicality sensory modality tense
